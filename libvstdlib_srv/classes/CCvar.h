// Generated header

class CCvar : public ICvar
{
public:
	virtual void Connect(_func_void_ptr_char_ptr_int_ptr *param_1); // vtable[0]
	virtual void Disconnect(); // vtable[1]
	virtual void QueryInterface(char *param_1); // vtable[2]
	virtual void Init(); // vtable[3]
	virtual void Shutdown() override; // vtable[4]
	virtual void AllocateDLLIdentifier(); // vtable[5]
	virtual void RegisterConCommand(ConCommandBase *param_1); // vtable[6]
	virtual void UnregisterConCommand(ConCommandBase *param_1); // vtable[7]
	virtual void UnregisterConCommands(intparam_1); // vtable[8]
	virtual void GetCommandLineValue(char *param_1); // vtable[9]
	virtual void FindCommandBase(char *param_1); // vtable[10]
	virtual void FindCommandBase(char *param_1); // vtable[11]
	virtual void FindVar(char *param_1); // vtable[12]
	virtual void FindVar(char *param_1); // vtable[13]
	virtual void FindCommand(char *param_1); // vtable[14]
	virtual void FindCommand(char *param_1); // vtable[15]
	virtual void GetCommands(); // vtable[16]
	virtual void GetCommands(); // vtable[17]
	virtual void InstallGlobalChangeCallback(_func_void_IConVar_ptr_char_ptr_float *param_1); // vtable[18]
	virtual void RemoveGlobalChangeCallback(_func_void_IConVar_ptr_char_ptr_float *param_1); // vtable[19]
	virtual void CallGlobalChangeCallbacks(ConVar *param_1, char *param_2, floatparam_3); // vtable[20]
	virtual void InstallConsoleDisplayFunc(IConsoleDisplayFunc *param_1); // vtable[21]
	virtual void RemoveConsoleDisplayFunc(IConsoleDisplayFunc *param_1); // vtable[22]
	virtual void ConsoleColorPrintf(Color *param_1, char *param_2); // vtable[23]
	virtual void ConsolePrintf(char *param_1); // vtable[24]
	virtual void ConsoleDPrintf(char *param_1); // vtable[25]
	virtual void RevertFlaggedConVars(intparam_1); // vtable[26]
	virtual void InstallCVarQuery(ICvarQuery *param_1); // vtable[27]
	virtual void IsMaterialThreadSetAllowed(); // vtable[28]
	virtual void QueueMaterialThreadSetValue(ConVar *param_1, char *param_2); // vtable[29]
	virtual void QueueMaterialThreadSetValue(ConVar *param_1, intparam_2); // vtable[30]
	virtual void QueueMaterialThreadSetValue(ConVar *param_1, floatparam_2); // vtable[31]
	virtual void HasQueuedMaterialThreadConVarSets(); // vtable[32]
	virtual void ProcessQueuedMaterialThreadConVarSets(); // vtable[33]
	virtual void FactoryInternalIterator(); // vtable[34]
	virtual void RevertServerConVars(boolparam_1) override; // vtable[35]

	void _GLOBAL__sub_I_SetFirst(); // size[119]
	void SetFirst(); // size[61]
	void IsValid(); // size[42]
	void Get(); // size[14]
	void Next(); // size[89]
	void DisplayQueuedMessages(); // size[656]
	void Find(CCommand *param_1); // size[993]
	CCvar(); // size[356]
	~CCVarIteratorInternal(); // size[14]
	~CCVarIteratorInternal(); // size[18]
	~CCvar(); // size[203]
};
