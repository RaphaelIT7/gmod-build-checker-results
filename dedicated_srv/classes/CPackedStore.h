// Generated header
// Estimated minimum size: 0x2CD8 (11480) bytes

class CPackedStore
{
public:
	void _GLOBAL__sub_I_FindFileEntry(); // size[48]
	void GetDataFileName(char *param_1, int param_2, int param_3); // size[73]
	void FindFileEntry(char *param_1, char *param_2, char *param_3, uchar * *param_4, uchar * *param_5); // size[317]
	void BuildHashTables(); // size[571]
	void IsEmpty(); // size[18]
	void GetDataFileName(char *param_1, int param_2, int param_3); // size[74]
	void OpenFile(char *param_1); // size[361]
	void GetHandleForHashingFiles(); // size[51]
	void DiscardChunkHashes(int param_1); // size[147]
	void ComputeDirectoryHash(MD5Value_t *param_1); // size[117]
	void ComputeChunkHash(MD5Value_t *param_1); // size[125]
	void BTestDirectoryHash(); // size[107]
	void BTestMasterChunkHash(); // size[82]
	void HashMetadata(); // size[52]
	void GetPackFileName(CPackedStoreFileHandle *param_1, char *param_2, int param_3); // size[29]
	void GetFileHandle(int param_1); // size[428]
	void HashEntirePackFile(CPackedStoreFileHandle *param_1, longlong *param_2, int param_3, int param_4, FileHash_t *param_5); // size[605]
	void InternalRemoveFileFromDirectory(char *param_1); // size[9]
	void RemoveFileFromDirectory(char *param_1); // size[4]
	void Init(); // size[201]
	void FindFileHashFraction(int param_1, int param_2, ChunkHashFraction_t *param_3); // size[150]
	void GetPackFileLoadErrorSummary(CUtlString *param_1); // size[493]
	~CPackedStore(); // size[2352]
	void HashChunkFile(int param_1); // size[569]
	void HashAllChunkFiles(); // size[102]
	void HashEverything(); // size[31]
	void Write(); // size[1145]
	void AddFileToDirectory(VPKContentFileInfo_t *param_1); // size[51]
	void GetFileList(char *param_1, CUtlStringList *param_2, bool param_3, bool param_4); // size[1439]
	void GetFileList(CUtlStringList *param_1, bool param_2, bool param_3); // size[50]
	CPackedStore(char *param_1, char *param_2, IBaseFileSystem *param_3, bool param_4); // size[3328]
	void AddFile(char *param_1, ushort param_2, void *param_3, uint param_4, bool param_5, uint *param_6); // size[1214]
	void GetFileList(char *param_1, CUtlVector *param_2); // size[497]
	void ReadData(CPackedStoreFileHandle *param_1, void *param_2, int param_3); // size[584]
	void BuildFindFirstCache(); // size[1219]
	void DirectoryEntryExists(char *param_1); // size[185]
	void GetFileAndDirLists(char *param_1, CUtlStringList *param_2, CUtlStringList *param_3, bool param_4); // size[2907]
	void GetFileAndDirLists(CUtlStringList *param_1, CUtlStringList *param_2, bool param_3); // size[49]
};
