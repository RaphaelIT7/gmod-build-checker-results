// Generated header
// Estimated minimum size: 0x248 (584) bytes, no debug info available

class CLuaNextBotVision : public IVision
{
public:
	virtual ~CLuaNextBotVision() override; // vtable[0]
	virtual ~CLuaNextBotVision() override; // vtable[1]
	virtual void GetMaxVisionRange() override; // vtable[59]
	virtual void SetMaxVisionRange(float param_1); // vtable[74]

	CLuaNextBotVision(CLuaNextBot *param_1); // size[68]
};
