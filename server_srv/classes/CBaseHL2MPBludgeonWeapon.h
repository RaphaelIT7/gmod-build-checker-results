// Generated header

class CBaseHL2MPBludgeonWeapon : public CBaseHL2MPCombatWeapon
{
public:
	virtual ~CBaseHL2MPBludgeonWeapon() override; // vtable[0]
	virtual ~CBaseHL2MPBludgeonWeapon() override; // vtable[1]
	virtual void GetServerClass() override; // vtable[10]
	virtual void YouForgotToImplementOrDeclareServerClass() override; // vtable[11]
	virtual void Spawn() override; // vtable[23]
	virtual void Precache() override; // vtable[24]
	virtual void ItemPostFrame() override; // vtable[318]
	virtual void PrimaryAttack() override; // vtable[332]
	virtual void SecondaryAttack() override; // vtable[333]
	virtual void GetPrimaryAttackActivity() override; // vtable[334]
	virtual void GetSecondaryAttackActivity() override; // vtable[335]
	virtual void GetFireRate() override; // vtable[341]
	virtual void GetRange(); // vtable[429]
	virtual void GetDamageForActivity(Activityparam_1); // vtable[430]
	virtual void ImpactEffect(CGameTrace *param_1); // vtable[431]

	void _GLOBAL__sub_I_GetServerClass(); // size[112]
	CBaseHL2MPBludgeonWeapon(); // size[37]
	void Hit(CGameTrace *param_1, Activityparam_2); // size[185]
	void ChooseIntersectionPointAndActivity(CGameTrace *param_1, Vector *param_2, Vector *param_3, CBasePlayer *param_4); // size[440]
	void ImpactWater(Vector *param_1, Vector *param_2); // size[396]
	void Swing(intparam_1); // size[254]
};
