// Generated header

class CTEMetalSparks : public CBaseTempEntity
{
public:
	virtual void GetServerClass() override; // vtable[0]
	virtual void _ZN14CTEMetalSparks40YouForgotToImplementOrDeclareServerClassEv() override; // vtable[1]
	virtual ~CTEMetalSparks() override; // vtable[2]
	virtual ~CTEMetalSparks() override; // vtable[3]
	virtual void Test(Vector *param_1, QAngle *param_2) override; // vtable[4]

	_GLOBAL__sub_I_CTEMetalSparks(); // size[113]
	CTEMetalSparks(char *param_1); // size[70]
};
